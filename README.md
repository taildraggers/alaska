...
Daily aggregator of Alaska-located taildragger classified listings,
...
repo's approach, with the state filter swapped from "AL" to "AK".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, AK` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Alaska"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/alaska/`
...
```html
<iframe
  src="https://taildraggers.github.io/alaska/"
  title="Taildragger Ads in Alaska"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_ALASKA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
