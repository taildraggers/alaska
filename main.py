"""Aggregates Alaska-located listings from every taildraggers.com
...
ones whose location ends in ", AK".
...
This repo is a straight copy of the companion Alabama repo's approach,
with the state filter swapped from "AL" to "AK".
"""
...
PAGE_TITLE = "Taildragger Ads in Alaska"
STATE_ABBR = "AK"
...
_ALASKA_LOCATION_RE = re.compile(r",\s*AK\s*$", re.IGNORECASE)


def _is_alaska(location: str) -> bool:
    return bool(_ALASKA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        alaska_rows = [row for row in rows if _is_alaska(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(alaska_rows)} in Alaska")
        all_listings.extend(alaska_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Alaska listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Alaska listings: {len(listings)}")
